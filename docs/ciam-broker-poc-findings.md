...
### 6. Parallel Okta OIDC connection under consideration

Okta supports both SAML and OIDC for federation and recommends OIDC for new application integrations. PingOne supports both protocols as generic external identity-provider types but does not publish an Okta-specific preference between them.

For this POC, a parallel Okta OIDC connection is a reasonable way to avoid the current SAML `NameIDPolicy` incompatibility without deleting the existing SAML connection. The expected implementation effort is approximately two to four hours when the Okta administrator is available. Most work is tenant configuration rather than application development.

The Okta-side work would include:

- Creating an OIDC Web Application that uses the Authorization Code flow.
- Registering the exact callback URL generated by PingOne.
- Assigning the required test users or groups.
- Providing the client ID and client secret securely to the PingOne administrator.
- Supplying standard identity claims and any required groups or council attributes.

The PingOne-side work would include:

- Creating a generic OIDC external IdP using Okta's discovery metadata.
- Configuring the Okta client credentials, scopes, token-endpoint authentication, and PKCE where supported.
- Mapping Okta claims and groups into the existing PingOne attributes and downstream roles.
- Pointing an authentication policy at the new OIDC provider.

Leader Tools should require no code change if the existing `OktaOnly` PingOne policy is updated to select the new provider. A parallel policy such as `OktaOidcOnly` would require only changing the `acr_values` policy name used by the **Council Sign In** action, or temporarily adding a separate POC test action.

OIDC is expected to avoid the SAML request-format problem, but it does not automatically solve universal logout. Testing must determine whether PingOne terminates the upstream Okta OIDC session or exhibits the same generic upstream OIDC logout limitation found with Gigya.
...
| Parallel Okta OIDC provider | Proposed | Estimated at two to four hours; expected to require zero or one application-code change. |
...
6. If the SAML workaround remains blocked or Signed Requests must remain enabled, create and test a parallel Okta OIDC external IdP.
7. For an OIDC connection, separately verify upstream Okta logout behavior rather than assuming that login success proves universal logout.
8. Rotate the exposed PingOne Worker secret.
